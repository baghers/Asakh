...
if ($login_Permission_granted==0) header("Location: ../login.php");
    ...
    if ($appfoundationID>0)
    {
        ...
        */
        ...
        $query = " DELETE FROM appfoundation where appfoundationID='$appfoundationID' and ApplicantMasterID='$ApplicantMasterID'
        and appfoundationID not in (select appfoundationID from manuallistpriceall where appfoundationID='$appfoundationID')
        and appfoundationID not in (select appfoundationID from manuallistprice where appfoundationID='$appfoundationID')
        ";
           	 	 			  	try 
								  {		
									    $result = mysql_query($query); 
